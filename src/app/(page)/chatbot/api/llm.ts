import { axios_llm } from "@/lib/axios";
...
export function chatBot() {
  ...
  function formatText(text: string): string {
    ...
    return text
      ...
      .replace(
        /-\s+(.*?)<br>/g,
        '<ul style="list-style-type:disc; margin-left:20px;"><li style="margin-left:20px;">$1</li></ul>',
      ) // Bullet points
  ...
  async function callConversation(message: string): Promise<void> {
    ...
    const skeletonCard: ChatMessage = {
      text: generateSkeletonText(),
      role: "bot",
    };
    setChatHistory([...updatedHistory, skeletonCard]);

    const formattedChatHistory = updatedHistory.map((chat) => [
      chat.role,
      chat.text,
    ]);
    ...
    axios_llm
      ...
      .then((response) => {
        ...
        const botMessage: ChatMessage = {
          text: formatText(parsedData.data.output),
          role: "bot",
        };
