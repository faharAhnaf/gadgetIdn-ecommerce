...
export default async function addCartItem(userId: string, productId: string, price: number, quantity: number) {
  try {
    ...
    const q = query(cartRef, where('user_id', '==', userRef), where('product_id', '==', productRef));
