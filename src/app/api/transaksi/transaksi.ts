...
const calculateTotal = ({ price, amount }: {price: number[], amount: number[]}): { total: number; totalQuantity: number } => {
    let total = 0
    let totalQuantity = 0;
...
const checkPaymentStatus = async (
    ...
    try {
        ...
        await Swal.fire('Error', 'An error occurred while checking payment status.', 'error');
    }
};
