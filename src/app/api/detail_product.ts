...
export const getProductByProductId = async (productId: string): Promise<Product | null> => {
    try {
        const productCollection = collection(db, "product");
        const productQuery = query(productCollection, where("product_id", "==", productId));
        const productSnap = await getDocs(productQuery);

        if (!productSnap.empty) {
            const productData = productSnap.docs[0].data();
            const product: Product = { product_id: productSnap.docs[0].id, ...productData } as Product;
